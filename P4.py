...
#SE TOMA LA DEF "MODULADOR" Y SE MODIFICA
def modulador(bits, fc, mpp):
    '''Un método que simula el esquema de 
    ...
    Tc = 1 / fc  # periodo [s]
    t_periodo = np.linspace(0, Tc, mpp)
    #Tenemos 2 portadoras
    port1 = np.sin(2*np.pi*fc*t_periodo)
    port2 = np.cos(2*np.pi*fc*t_periodo) 
    ...
    for i, bit in enumerate(bits):
        ...
        if (bit != 0) and (cont%2 != 0):
            senal_Tx[i*mpp : (i+1)*mpp] = port1*1
            moduladora[i*mpp : (i+1)*mpp] = 1
            cont = cont + 1
          
        elif (bit == 0) and (cont%2 != 0):
            senal_Tx[i*mpp : (i+1)*mpp] = port1*-1
            moduladora[i*mpp : (i+1)*mpp] = 0
            cont = cont + 1
         
        elif (bit != 0) and (cont%2 == 0):
            senal_Tx[i*mpp : (i+1)*mpp] = port2*1
            moduladora[i*mpp : (i+1)*mpp] = 1
            cont = cont + 1
    ...
    # 5. Calcular la potencia promedio de la señal modulada
    Pm = (1 / (N*Tc)) * np.trapz(pow(senal_Tx, 2), t_simulacion)
    #Se suman las port1 y port2

    port = port1 + port2
    
    return senal_Tx, Pm, port, moduladora  


# In[20]:


# SE TOMA LA DEF "DEMODULADOR" Y SE MODIFICA

def demodulador(senal_Rx, port, mpp):
    '''Un método que simula un bloque demodulador
    ...
    for i in range(N):
        # Producto interno de dos funciones
        producto = senal_Rx[i*mpp : (i+1)*mpp] * port
        senal_demodulada[i*mpp : (i+1)*mpp] = producto
        Ep = np.sum(producto) 
...
# SIMULACIÓN TOMANDO SECCIÓN 3.2 Y MODIFICANDO
...
# 3. Modular la cadena de bits usando el esquema BPSK
senal_Tx, Pm, port, moduladora = modulador(bits_Tx, fc, mpp)

# 4. Se transmite la señal modulada, por un canal ruidoso
senal_Rx = canal_ruidoso(senal_Tx, Pm, SNR)

# 5. Se desmodula la señal recibida del canal
bits_Rx, senal_demodulada = demodulador(senal_Rx, port, mpp)
...
#Vector tiempo
time = np.linspace(0, 0.09,150)

Amp=[1,-1]
X_t = np.empty((4, len(time)))	   # 4 funciones del tiempo x(t) (N funciones con T puntos)
plt.figure()

#Matriz de posibles funciones  
for i in Amp:
    A = i * np.cos(2*(np.pi)*fc*time) +  i* np.sin(2*(np.pi)*fc*time)
    Z = -i * np.cos(2*(np.pi)*fc*time) +  i* np.sin(2*(np.pi)*fc*time) 
    X_t[i,:] = A
    X_t[i+1,:] = Z
    plt.plot(time, A , lw=2)
    plt.plot(time, Z , lw=2)       

# Promedio de las N realizaciones en cada instante (cada punto en t)
P = [np.mean(X_t[:,i]) for i in range(len(time))]
plt.plot(time, P, lw=6,label='Resultado Promedio')

# Graficar el resultado teórico del valor esperado
E = np.mean(senal_Tx)*time
plt.plot(time, E, '-.', lw=3,label='Resultado teórico')
...
print("En la señal se presenta ergocicidad debido a que el promedio y valor teórico coinciden")
...
# Número de símbolos (198pix x 89 x 8bits x 3canales)
Ns = Nm // mpp
